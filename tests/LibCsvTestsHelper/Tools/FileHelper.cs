...
public static class FileHelper
{
	...
	///		Obtiene un CSV reader de un archivo
	/// </summary>
	public static CsvReaderNotOptimized GetCsvReaderNotOptimized(string file)
	{
		SchemaModel? schema = GetSchema(file);

			if (schema is null)
				return new CsvReaderNotOptimized(new FileModel(), null);
			else
				return new CsvReaderNotOptimized(schema.Definition, GetCsvColumns(schema.Columns));
	}
